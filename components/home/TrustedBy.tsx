    ...
    <section className="border-t border-green-950/10 bg-cream-100 py-12 sm:py-14">
      ...
      <div className="relative mt-8 flex overflow-hidden [mask-image:linear-gradient(to_right,transparent,black_8%,black_92%,transparent)]">
        <div className="flex w-max shrink-0 animate-marquee items-center gap-20 pr-20 motion-reduce:animate-none">
          {track.map((client, i) => (
            <div key={`${client.name}-${i}`} className="flex h-20 w-40 shrink-0 items-center justify-center">
              <Image
                src={client.logo}
                alt={client.name}
                width={client.width}
                height={client.height}
                className="h-auto max-h-20 w-auto max-w-40 object-contain opacity-80 transition-opacity duration-300 hover:opacity-100"
              />
            </div>
          ))}
        </div>
